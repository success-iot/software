...
class Messages extends Resource
  {
    ...
    */
    ...
    function PUT($key, $input){
         ...
         if(!isPrimaryUser() || !isset($input['readed'])){
            return 0;
         }
         $readed = $input['readed'];
         if (!isset($readed)){
        ...
        //JUST leave mark message as read, no other modification are allowed
          $this->query =  "update " . $this->table . " SET readed=$readed where " . $this->resourcekey . "=" .$key . " and " . $this->userKey  . "=" . $this->session->userid;
...
/** idUser is ommited for security reasons, getted from session*/
   ...
   */
   ...
   function broadCastRead(){
      ...
      $msg = "User " .  $this->session->username . " access to Outcomes.";
      foreach($ids as $id){
        $this->createMessage($id, 2, $msg);
      }
   }
}
